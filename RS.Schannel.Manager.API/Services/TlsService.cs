...
internal sealed class TlsService : ITlsService
{
    ...
    public TlsService(IWindowsCipherSuiteDocumentationService windowsCipherSuiteDocumentationService)
    {
    ...
    public async Task GetRemoteServerCipherSuitesAsync(string hostName, CancellationToken cancellationToken)
    {
        WindowsSchannelVersion windowsSchannelVersion = GetWindowsSchannelVersion();
        List<WindowsDocumentationCipherSuiteConfiguration> windowsDocumentationCipherSuiteConfigurations = windowsCipherSuiteDocumentationService.GetWindowsDocumentationCipherSuiteConfigurations(windowsSchannelVersion);
        IPAddress[] ipAddresses = await Dns.GetHostAddressesAsync(hostName, cancellationToken);
        var ipEndPoint = new IPEndPoint(ipAddresses.First(q => (Socket.OSSupportsIPv6 && q.AddressFamily is AddressFamily.InterNetworkV6) || q.AddressFamily is AddressFamily.InterNetwork), 443);
        var result = new List<(SslProviderProtocolId, List<(SslProviderCipherSuiteId, bool, TlsAlert?)>?)>();

        foreach (SslProviderProtocolId sslProviderProtocolId in Enum.GetValues<SslProviderProtocolId>().Where(q => q is not SslProviderProtocolId.DTLS1_0_PROTOCOL_VERSION and not SslProviderProtocolId.DTLS1_2_PROTOCOL_VERSION).Distinct())
        {
            List<(SslProviderCipherSuiteId, bool, TlsAlert?)>? sslProviderProtocolIdResult = null;

            try
            {
                sslProviderProtocolIdResult = await GetRemoteServerCipherSuitesAsync(ipEndPoint, hostName, sslProviderProtocolId, windowsDocumentationCipherSuiteConfigurations, cancellationToken);
            }
    ...
    [SupportedOSPlatform("windows")]
    private static async Task<List<(SslProviderCipherSuiteId, bool, TlsAlert?)>> GetRemoteServerCipherSuitesAsync(IPEndPoint ipEndpoint, string hostName, SslProviderProtocolId sslProviderProtocolId, List<WindowsDocumentationCipherSuiteConfiguration> windowsDocumentationCipherSuiteConfigurations, CancellationToken cancellationToken)
    {
        TlsVersion tlsVersion = sslProviderProtocolId switch
        {
            SslProviderProtocolId.SSL2_PROTOCOL_VERSION => TlsVersion.SSL2_PROTOCOL_VERSION,
            SslProviderProtocolId.SSL3_PROTOCOL_VERSION => TlsVersion.SSL3_PROTOCOL_VERSION,
            SslProviderProtocolId.TLS1_PROTOCOL_VERSION => TlsVersion.TLS1_0_PROTOCOL_VERSION,
            SslProviderProtocolId.TLS1_1_PROTOCOL_VERSION => TlsVersion.TLS1_1_PROTOCOL_VERSION,
            SslProviderProtocolId.TLS1_2_PROTOCOL_VERSION => TlsVersion.TLS1_2_PROTOCOL_VERSION,
            SslProviderProtocolId.TLS1_3_PROTOCOL_VERSION => TlsVersion.TLS1_3_PROTOCOL_VERSION,
            _ => throw new ArgumentOutOfRangeException(nameof(sslProviderProtocolId), sslProviderProtocolId, null)
        };
        SslProviderCipherSuiteId[] sslProviderCipherSuiteIds = Enum.GetValues<SslProviderCipherSuiteId>();
        ...
        var results = new List<(SslProviderCipherSuiteId, bool, TlsAlert?)>();

        // https://www.rfc-editor.org/rfc/rfc2246#appendix-E
        // E.1. Version 2 client hello
        foreach (SslProviderCipherSuiteId sslProviderCipherSuiteId in sslProviderCipherSuiteIds)
        {
            byte[] clientHelloBytes;

            if (tlsVersion is TlsVersion.SSL2_PROTOCOL_VERSION)
            {
                var ssl2ClientHelloRecord = new Ssl2ClientHelloRecord(new[] { sslProviderCipherSuiteId });

                clientHelloBytes = ssl2ClientHelloRecord.GetBytes();
            }
            else
            {
                var clientHelloTlsRecord = new ClientHelloTlsRecord(hostName, tlsVersion, new[] { sslProviderCipherSuiteId }, tlsCompressionMethodIdentifiers, tlsEllipticCurvesPointFormats, tlsSignatureSchemes, tlsSupportedGroups, new[] { tlsVersion }, tlsPreSharedKeysKeyExchangeModes, keyShares);
            ...
            if (!responseBytes.Span.IsEmpty && (responseBytes.Span[0] & 0x80) == 0x80)
            {
